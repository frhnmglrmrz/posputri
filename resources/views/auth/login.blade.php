...
<div class="sm:mx-auto sm:w-full sm:max-w-md px-4">
    ...
    <div class="text-center mb-8">
        ...
        <h2 class="text-2xl font-bold tracking-tight text-slate-900">Berkah Mart Offline-First</h2>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-xl">
        <form method="POST" action="{{ route('login') }}" class="space-y-4">
            ...
            <div>
                ...
                <input id="email" name="email" type="email" autocomplete="email" required value="{{ old('email', 'admin@berkahmart.test') }}"
        ...
        <div class="mt-6 pt-6 border-t border-slate-200 text-xs">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <button type="button" onclick="document.getElementById('email').value='kasir@berkahmart.test'" class="p-2.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 border border-emerald-300 text-center transition active:scale-95 group">
                    <div class="font-bold text-emerald-800">Kasir</div>
                    <div class="text-[10px] text-emerald-600 truncate">kasir@...</div>
                </button>
                <button type="button" onclick="document.getElementById('email').value='gudang@berkahmart.test'" class="p-2.5 rounded-lg bg-cyan-50 hover:bg-cyan-100 border border-cyan-300 text-center transition active:scale-95 group">
                    <div class="font-bold text-cyan-800">Gudang</div>
                    <div class="text-[10px] text-cyan-600 truncate">gudang@...</div>
                </button>
                <button type="button" onclick="document.getElementById('email').value='supervisor@berkahmart.test'" class="p-2.5 rounded-lg bg-amber-50 hover:bg-amber-100 border border-amber-300 text-center transition active:scale-95 group">
                    <div class="font-bold text-amber-900">Supervisor</div>
                    <div class="text-[10px] text-amber-700 truncate">spv@...</div>
                </button>
                <button type="button" onclick="document.getElementById('email').value='admin@berkahmart.test'" class="p-2.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 border border-indigo-300 text-center transition active:scale-95 group">
    ...
    <div class="text-center mt-6">
        <a href="{{ url('/') }}" class="text-xs text-slate-500 hover:text-slate-800 transition flex items-center justify-center gap-1.5 font-medium">
            ...
            <span>Kembali ke Beranda Berkah Mart</span>
        </a>
    </div>
</div>
@endsection
